...
<?php if(isset($_SESSION["felhasznalo"]) && $_SESSION["felhasznalo"]["role"] === 'admin' ){
    ...
    $select = 'SELECT KAR_ID as "ID" , KAR_NEV , KAR_KOD
                FROM "Kar"';
    ...
    echo printf('<form action="a_karok_add.php" method="post">
                                        <tr>
                                            <td>Új Kar</td>
                                            <td>
                                                <input type="text" name="KAR_NEV" id="KAR_NEV" >
                                            </td>
                                            <td>
                                                <input type="text" name="KAR_KOD" id="KAR_KOD" >
                                            </td>
                                            
                                            <td>
                                                <input class="submit" type="submit" value="Add Kar">
                                            </td>
                                            </tr>
                                    </form>');

    while ($record = oci_fetch_array($params[0], OCI_ASSOC + OCI_RETURN_NULLS)) {
        echo sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td><a href="a_mod_del_karok.php?value=' . urlencode($record['ID']) . '">Adatok módostása/törlése</a></td></tr>', $record['ID'], $record['KAR_NEV'], $record['KAR_KOD']);
    }
