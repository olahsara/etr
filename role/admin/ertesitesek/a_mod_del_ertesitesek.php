...
<head>
    <meta charset="UTF-8">
    <title>Értesitések módosit</title>
...
<div style="margin-top: 100px">
    <?php if(isset($_SESSION["felhasznalo"]) && $_SESSION["felhasznalo"]["role"] === 'admin' ){
        ...
        if (isset($_GET['value'])) {
            ...
            $select = 'SELECT ERTESITES_ID as "ID", ERTESITES_IDOPONT ,UZENET
                        FROM "Ertesites"
                        WHERE ERTESITES_ID=' . $value;
            ...
            while ($record = oci_fetch_array($params[0], OCI_ASSOC + OCI_RETURN_NULLS)) {
                ...
                echo sprintf('
                                    <form action="a_ertesitesek_mod.php?value=' . urlencode($record['ID']) . '" method="post">
                                        <tr>
                                            <td>%s</td>
                                            ...
                                            <td>
                                                <input type="text" name="UZENET" id="UZENET" value=%s>
                                            </td>
                                            
                                            <td>
                                                <input class="submit" type="submit" value="Update Ertesites">
                                                <a href="a_ertesitesek_del.php?value=' . urlencode($record['ID']) . '">Delete Ertesites</a>
                                            </td>
                                            </tr>
                                    </form>', $record['ID'], $record['ERTESITES_IDOPONT'], $record['UZENET']);
            }
