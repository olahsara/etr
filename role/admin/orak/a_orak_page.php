...
<?php if(isset($_SESSION["felhasznalo"]) && $_SESSION["felhasznalo"]["role"] === 'admin' ){
    ...
    echo printf('<form action="a_orak_add.php" method="post">
                                        <tr>
                                            ...
                                            <td>
                                                <input class="submit" type="submit" value="Add Ora">
                                            </td>
                                            </tr>
                                    </form>');

    while ($record = oci_fetch_array($params[0], OCI_ASSOC + OCI_RETURN_NULLS)) {
        echo sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td><a href="a_mod_del_orak.php?value=' . urlencode($record['ID']) . '">Adatok módostása/törlése</a></td></tr>', $record['ID'], $record['NAP'], $record['ORA']);
    }
