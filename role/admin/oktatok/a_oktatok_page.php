...
<?php if(isset($_SESSION["felhasznalo"]) && $_SESSION["felhasznalo"]["role"] === 'admin' ){
    ...
    echo printf('<form action="a_oktatok_add.php" method="post">
                                        <tr>
                                            <td>Új Oktato</td>
                                            <td>
                                                <input type="text" name="OKTATO_NEV" id="OKTATO_NEV" >
                                            </td>
                                            <td>
                                                <input type="text" name="JELSZO" id="JELSZO" >
                                            </td>
                                            <td>
                                                <input type="text" name="BEOSZTAS" id="BEOSZTAS" >
                                            </td>
                                            <td>
                                                <input type="text" name="NEPTUN_KOD" id="NEPTUN_KOD" >
                                            </td>
                                            
                                            
                                            <td>
                                                <input class="submit" type="submit" value="Add Oktato">
                                            </td>
                                            </tr>
                                    </form>');

    while ($record = oci_fetch_array($params[0], OCI_ASSOC + OCI_RETURN_NULLS)) {
        echo sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href="a_mod_del_oktatok.php?value=' . urlencode($record['ID']) . '">Adatok módostása/törlése</a></td></tr>', $record['ID'], $record['OKTATO_NEV'], $record['JELSZO'], $record['BEOSZTAS'], $record['NEPTUN_KOD']);
    }
