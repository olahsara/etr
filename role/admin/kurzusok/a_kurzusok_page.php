...
<?php if(isset($_SESSION["felhasznalo"]) && $_SESSION["felhasznalo"]["role"] === 'admin' ){
    ...
    echo printf('<form action="a_kurzusok_add.php" method="post">
                                        <tr>
                                            <td>Új Kurzus</td>
                                            <td>
                                                <input type="text" name="KURZUS_KOD" id="KURZUS_KOD" >
                                            </td>
                                            <td>
                                                <input type="text" name="KURZUS_NEV" id="KURZUS_NEV" >
                                            </td>
                                            <td>
                                                <input type="text" name="AJANLOTT_FELEV" id="AJANLOTT_FELEV" >
                                            </td>
                                            <td>
                                                <input type="text" name="KREDIT" id="KREDIT" >
                                            </td>
                                            <td>
                                                <input type="text" name="ORASZAM" id="ORASZAM" >
                                            </td>
                                            <td>
                                                <input type="text" name="MEGNYITVA" id="MEGNYITVA" >
                                            </td>
                                            
                                            
                                            <td>
                                                <input class="submit" type="submit" value="Add Kurzus">
                                            </td>
                                            </tr>
                                    </form>');

    while ($record = oci_fetch_array($params[0], OCI_ASSOC + OCI_RETURN_NULLS)) {
        echo sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href="a_mod_del_kurzusok.php?value=' . urlencode($record['ID']) . '">Adatok módostása/törlése</a></td></tr>', $record['ID'], $record['KURZUS_KOD'], $record['KURZUS_NEV'], $record['AJANLOTT_FELEV'], $record['KREDIT'], $record['ORASZAM'], $record['MEGNYITVA']);
    }
